...
\begin{frame}\frametitle{Subset extraction}
  ...
  \begin{block}{Extraction algorithm}
    A multiplex network $ \Psi $ is decomposed into individual communities via the use of $Louvain$ algorithm. Initial term list $\psi $ is then splitted according to community presence, such that initial terms $ t_{1},t_{2},..,t_{n} $ are assigned to a subset $\phi \in \psi$ $iff$ $t_{x..y} \in \Psi_{x..y}$.
  \end{block}
  \begin{center}
